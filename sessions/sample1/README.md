...
Formerly dataset 1 of `lctk_sample_data`: a VLP-32C `lidar.pcap` and a camera
...
`ros2 launch lctk_launch session.launch.py session:=sessions/sample1` and produced
...
- `data/` — the recording: `lidar.pcap` and `video.avi`. It moved here from
  `ros/lctk_sample_data/data/1` so the session is self-contained.
