...
#[cfg(feature = "with-tch")]
mod with_tch {
    use crate::{ext::ImageFormat, ColorSpace, Image};
    ...
    #[derive(Debug)]
    pub struct TensorWithColor {
        pub tensor: tch::Tensor,
        pub color: ColorSpace,
    }

    impl TryFrom<Image> for TensorWithColor {
        type Error = Error;
    ...
    impl TryFrom<&Image> for TensorWithColor {
        ...
        fn try_from(image: &Image) -> Result<Self, Self::Error> {
            ...
            let (tensor, color) = match format {
                ImageFormat::RGB3 => {
                    ensure!(data.len() == (height * width * channels) as usize);
                    ensure!(channels == 3);
                    let tensor = tch::Tensor::of_slice(data)
                ...
                ImageFormat::BGR3 => {
                    ...
                    let tensor = tch::Tensor::of_slice(data)
                        .view([height as i64, width as i64, channels as i64])
                        .permute(&[2, 0, 1]);
                    let color = ColorSpace::Bgr;
                    (tensor, color)
                }
            };

            Ok(Self { tensor, color })
        }
    }

    impl TryFrom<TensorWithColor> for Image {
        type Error = Error;

        fn try_from(from: TensorWithColor) -> Result<Self, Self::Error> {
            (&from).try_into()
        }
    }
    ...
    impl TryFrom<&TensorWithColor> for Image {
        ...
        fn try_from(from: &TensorWithColor) -> Result<Self, Self::Error> {
            let TensorWithColor { ref tensor, color } = *from;

            let image = match color {
                ColorSpace::Rgb => {
                    let (channels, height, width) = tensor.size3()?;
                    ensure!(channels == 3);
                    ensure!(tensor.kind() == tch::Kind::Uint8);

                    let tensor = tensor.permute(&[1, 2, 0]).to_device(tch::Device::Cpu);

                    let data = unsafe {
                        let len = (height * width * channels) as usize;
                        let slice = slice::from_raw_parts(tensor.data_ptr() as *const u8, len);
                        slice.to_vec()
                    };

                    Self {
                        fourcc: ImageFormat::RGB3.to_fourcc().to_vec(),
                        height: height as u64,
                        width: width as u64,
                ...
                ColorSpace::Bgr => {
                    ...
                    Self {
                        fourcc: ImageFormat::BGR3.to_fourcc().to_vec(),
                        height: height as u64,
                        width: width as u64,
                        channels: channels as u64,
                        data,
    ...
    mod tests {
        ...
        #[test]
        fn convert_image_tensor() -> Result<()> {
            let mut rng = rand::thread_rng();
            ...
            let tensor: TensorWithColor = (&input).try_into()?;
            ensure!(tensor.color == ColorSpace::Rgb);
            ensure!(*tensor.tensor.size() == [CHANNELS as i64, HEIGHT as i64, WIDTH as i64]);
            ensure!(tensor.tensor.kind() == tch::Kind::Uint8);

            let output: Image = tensor.try_into()?;
            ensure!(input == output);
