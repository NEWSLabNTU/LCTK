...
mod types {
    ...
    pub struct BoardDetectorConfig {
        ...
        pub plane_ransac_inlier_threshold: R64,
        #[serde(flatten)]
    ...
    impl BoardDetector {
        pub fn new(config: BoardDetectorConfig, aruco_pattern: MultiArucoPattern) -> Self {
            Self {
                config,
                aruco_pattern,
            }
...
pub fn fit_plane_ransac<'a>(
    board_detector: &BoardDetectorConfig,
    ...
    let mut arrsac = Arrsac::new(plane_ransac_inlier_threshold.raw(), rand::thread_rng())
...
pub fn fit_board_icp(
    ...
    // reject result if loss is too large
    {
        ...
        if min_icp_loss > icp_rejection_threshold.raw() {
            return Ok(None);
        }
    }
