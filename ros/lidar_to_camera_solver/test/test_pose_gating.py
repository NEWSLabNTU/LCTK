"""M-12: one bad pose in the buffer must not corrupt the whole calibration.

Ported from main's advanced_extrinsic_solver suite when this branch rebased onto
main (H-16); the solver was renamed and its rejection gate had to be reimplemented
against DetectionBuffer, so these are that port's acceptance criteria.
...
import cv2
import numpy as np
from lidar_to_camera_solver.detection_buffer import reject_outlier_poses
...
def test_a_quarter_turned_pose_is_rejected():
    ...
    rejected, rms = reject_outlier_poses(objs, imgs, K, rvec, tvec)
...
def test_a_clean_buffer_rejects_nothing():
    ...
    rejected, _ = reject_outlier_poses(objs, imgs, K, rvec, tvec)
...
def test_rejecting_the_bad_pose_improves_the_extrinsic():
    ...
    rejected, _ = reject_outlier_poses(objs, imgs, K, rvec_all, tvec_all)
...
def test_never_rejects_below_the_minimum_kept():
    ...
    rejected, _ = reject_outlier_poses(objs, imgs, K, rvec, tvec, min_keep=3)
...
def test_reports_per_pose_rms_for_every_pose():
    ...
    _, rms = reject_outlier_poses(objs, imgs, K, rvec, tvec)
