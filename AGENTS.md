...
This is the canonical project guide: build system, known issues, calibration
workflow, coding guidelines, and agent workflow all live here.
...
- Work on a `fix/...`, `feat/...`, or `docs/...` branch. Verify it, then
  fast-forward it into `main` with `git checkout main && git merge --ff-only
  <branch>` before pushing.
- Verify before claiming completion: `just build` and `just test` must pass;
  Python edits also require `just lint-py`. Paste-worthy evidence beats
  assertion.
- Multiple agents may work in this repository. Before taking an issue, check
  `docs/issues/README.md` for 🟡 (in-progress) markers; do not take an issue
  another agent holds. Always fetch and rebase before pushing because
  `origin/main` may have moved.
- Let the owning tool handle its domain. Autoware transform-direction and
  frame-convention handling belongs in `lctk_autoware_export`; ad-hoc
  inversions elsewhere reintroduce M-01-class bugs.

## Current State Pointers

- Open issues and statuses: `docs/issues/README.md`.
- Remaining open issues need hardware/operator work (field data or RViz
  verification); check the tracker before assuming an issue is fixable
  headlessly.
- The `ros/conflux` submodule is maintained by the repository owner. Its
  lockfile churn from workspace builds is intentionally hidden (`ignore =
  dirty`) and must never be committed upstream.
...
The expected development baseline is Ubuntu 22.04, ROS 2 Humble, and the
system Python installation. Keep Python dependencies compatible with that
environment; in particular, never install ROS Python packages with
`pip3 install --user`.
...
- When an issue closes, **add a resolution note**, move the file to
  `docs/issues/archive/`, and repair every relative link that crosses the move (both directions).
  Verify with a link check: every `](...*.md)` target under `docs/` must exist
- Multi-step remediations get a phase doc in `docs/roadmap/`; designs worth reviewing land in
