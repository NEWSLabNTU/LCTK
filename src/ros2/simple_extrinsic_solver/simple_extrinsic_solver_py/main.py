...
class SimpleExtrinsicSolver(Node):
    ...
    def __init__(self):
        ...
        self.aruco_pattern_file = self.get_parameter('aruco_pattern_file').get_parameter_value().string_value
        self.enable_quality_assessment = self.get_parameter('enable_quality_assessment').get_parameter_value().bool_value
        # Get config file paths, use defaults if empty
        aruco_config_file = self.get_parameter('aruco_config_file').get_parameter_value().string_value
        board_detector_file = self.get_parameter('board_detector_file').get_parameter_value().string_value
        ...
        self.aruco_config_file = aruco_config_file if aruco_config_file else os.path.join(workspace_root, 'config', 'aruco_pattern.json5')
        self.board_detector_file = board_detector_file if board_detector_file else os.path.join(workspace_root, 'config', 'board_detector.json5')
        self.intrinsics_file = intrinsics_file if intrinsics_file else os.path.join(workspace_root, 'config', 'intrinsics.yaml')
        ...
        self.debug_aruco_publisher = self.create_publisher(
            Detection2DArray, 
            'debug/recent_aruco_detections', 
            qos_profile
        )
        self.debug_board_publisher = self.create_publisher(
            Detection3DArray, 
            'debug/recent_board_detections', 
            qos_profile
        )
    ...
    def _load_aruco_pattern(self) -> Dict[str, Any]:
        ...
        if self.aruco_config_file:
            try:
                ...
                self.get_logger().warn(f"Failed to load ArUco config file '{self.aruco_config_file}': {e}")
    ...
    def aruco_callback(self, msg: Detection2DArray):
        ...
        if msg.detections:
            self.get_logger().info(f"Caching non-empty ArUco detection with {len(msg.detections)} markers")
    ...
    def board_callback(self, msg: Detection3DArray):
        ...
        # Only cache non-empty detections
        if msg.detections:
            self.get_logger().info(f"Caching non-empty board detection with {len(msg.detections)} boards")
    ...
    def _try_process_cached_detections(self):
        ...
        if aruco_detection and board_detection:
            ...
            try:
                ...
                if solved:
                    self.get_logger().info("Successfully processed cached detection pair - SOLUTION COMPUTED!")
            except Exception as e:
    ...
    def _process_detection_pair(self, aruco_msg: Detection2DArray, board_msg: Detection3DArray) -> bool:
        ...
        if not self.camera_info:
            self.get_logger().error("Camera info not available - cannot proceed with calibration")
            return False
        ...
        object_points, image_points = self._create_point_correspondences(aruco_markers, board_detection)
        
        if len(object_points) == 0:
            self.get_logger().error("No point correspondences created - cannot solve PnP")
            return
        
        self.get_logger().info(f"Created {len(object_points)} point correspondences for PnP solving")
        ...
        if success:
            ...
            if self.enable_quality_assessment:
                self._assess_quality(object_points, image_points, rvec, tvec, aruco_msg, board_msg)
            return True
        else:
            self.get_logger().error("PnP solver FAILED to find solution")
            return False
    
    def _detection2d_to_aruco_markers(self, detection_msg: Detection2DArray) -> List[ArUcoMarker]:
    ...
    def _create_point_correspondences(self, aruco_markers: List[ArUcoMarker], 
                                    board_detection: BoardDetection) -> Tuple[List[np.ndarray], List[np.ndarray]]:
    ...
    def _solve_pnp(self, object_points: List[np.ndarray], image_points: List[np.ndarray]) -> Tuple[bool, np.ndarray, np.ndarray]:
    ...
    def _create_transform_message(self, rvec: np.ndarray, tvec: np.ndarray, header: Header) -> TransformStamped:
    ...
    def _rotation_matrix_to_quaternion(self, rotation_matrix: np.ndarray) -> np.ndarray:
        ...
        return np.array([float(x), float(y), float(z), float(w)])  # Return as (x, y, z, w)
    ...
    def _assess_quality(self, object_points: List[np.ndarray], image_points: List[np.ndarray],
                       rvec: np.ndarray, tvec: np.ndarray, aruco_msg: Detection2DArray, 
        ...
        try:
            ...
            # Compute reprojection errors
            errors = []
            for i, (original, projected) in enumerate(zip(image_points, projected_points)):
            ...
            detection_confidence = min(len(aruco_msg.detections) / expected_detections, 1.0)
            
            # Create quality report
            quality_report = {
                "overall_quality": max(0.0, 1.0 - mean_error / 10.0),  # Simple quality metric
                "metrics": {
