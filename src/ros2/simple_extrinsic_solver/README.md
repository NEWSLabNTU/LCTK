...
ros2 run simple_extrinsic_solver_py simple_extrinsic_solver

# Or run with launch file
ros2 launch simple_extrinsic_solver_py simple_extrinsic_solver.launch.py
```
...
```bash
cd src/ros2
colcon build --packages-select simple_extrinsic_solver_py --symlink-install
```
...
```bash
source install/setup.sh
timeout 5s ros2 run simple_extrinsic_solver_py simple_extrinsic_solver
```
