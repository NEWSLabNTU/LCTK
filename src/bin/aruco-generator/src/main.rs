...
fn main() -> Result<()> {
    ...
    if let Some(config_path) = &args.config {
        ...
        config.validate()?;
        run_from_config(config, args.preview)?;
        return Ok(());
    }
    ...
    let config = match choice {
        0 => interact_single_aruco(&mut term)?,
        1 => interact_single_charuco(&mut term)?,
        2 => interact_multiple_arucos(&mut term)?,
        _ => unreachable!(),
    };

    config.validate()?;
    config.generate_image(args.preview)?;
...
fn interact_multiple_arucos(term: &mut Term) -> Result<Config> {
    ...
    let marker_ids_vec = query_marker_ids(term, n_markers, dict_size)?;
...
fn run_from_config(config: Config, preview: bool) -> Result<()> {
    config.generate_image(preview)
}

pub fn generate_single_charuco_image(config: &SingleCharucoConfig, preview: bool) -> Result<()> {
...
pub fn generate_multiple_arucos_image(config: &MultipleArucosConfig, preview: bool) -> Result<()> {
    let dictionary = &config.dictionary;
    let opencv_dictionary = dictionary.to_opencv_dictionary()?;
