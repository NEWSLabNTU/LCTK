...
class CalibrationValidator(Node):
    def __init__(self, args):
        ...
        self.get_logger().info(f"Validation started - waiting up to {args.timeout}s for calibration")
    ...
    def validate_transform(self, transform: TransformStamped):
        ...
        self.get_logger().info(f"Rotation: ({rotation.x:.3f}, {rotation.y:.3f}, {rotation.z:.3f}, {rotation.w:.3f})")
        self.get_logger().info(f"Rotation angle: {rotation_angle:.3f} rad ({math.degrees(rotation_angle):.1f}°)")
        
        # Validate ranges
        translation_valid = self.translation_range[0] <= translation_magnitude <= self.translation_range[1]
        rotation_valid = self.rotation_range[0] <= rotation_angle <= self.rotation_range[1]
        
        # Report validation results
        if translation_valid:
            self.get_logger().info(f"✅ Translation magnitude VALID ({translation_magnitude:.3f}m within {self.translation_range})")
        else:
            self.get_logger().error(f"❌ Translation magnitude INVALID ({translation_magnitude:.3f}m outside {self.translation_range})")
        
        if rotation_valid:
            self.get_logger().info(f"✅ Rotation angle VALID ({rotation_angle:.3f} rad within {self.rotation_range})")
        else:
            self.get_logger().error(f"❌ Rotation angle INVALID ({rotation_angle:.3f} rad outside {self.rotation_range})")
    ...
    def check_timeout(self):
        ...
        if elapsed > self.args.timeout:
            if self.calibration_received:
                self.get_logger().warn("Timeout reached, but calibration was received and validated")
            else:
                self.get_logger().error(f"⏰ TIMEOUT: No calibration received within {self.args.timeout}s")
                sys.exit(2)  # Timeout failure


def main():
    parser = argparse.ArgumentParser(description='Validate multi_wayside_node calibration')
    parser.add_argument('--timeout', type=int, default=60,
                        help='Timeout in seconds to wait for calibration')
    parser.add_argument('--expected_translation_range', type=str, default='0.5,10.0',
                        help='Expected translation magnitude range as "min,max"')
    parser.add_argument('--expected_rotation_range', type=str, default='0.0,1.57',
