...
class PerformanceProfiler(Node):
    def __init__(self, duration_sec=60, output_dir='performance_results'):
        ...
        self.subs = {
            'lidar1_points': self.create_subscription(
                PointCloud2, '/lidar1/points', 
                lambda msg: self.on_pointcloud(msg, 'lidar1'), qos),
            'lidar2_points': self.create_subscription(
                PointCloud2, '/lidar2/points',
                lambda msg: self.on_pointcloud(msg, 'lidar2'), qos),
            'lidar1_detection': self.create_subscription(
                Detection3DArray, '/lidar1/board_detection',
                lambda msg: self.on_detection(msg, 'lidar1'), qos),
            'lidar2_detection': self.create_subscription(
                Detection3DArray, '/lidar2/board_detection',
                lambda msg: self.on_detection(msg, 'lidar2'), qos),
            'calibration': self.create_subscription(
                TransformStamped, '/calibration_transform',
                self.on_calibration, qos),
            'rosout': self.create_subscription(
                Log, '/rosout',
                self.on_log, qos)
        }
        ...
        self.get_logger().info(f"Started performance profiling for {duration_sec} seconds")
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                ...
                if self.process_name in cmdline:
                    ...
                    self.get_logger().info(f"Found {self.process_name} process (PID: {proc.pid})")
                    return True
    ...
    def on_calibration(self, msg: TransformStamped):
        ...
        self.get_logger().info(f"Calibration #{self.metrics['calibration_count']} received")
    ...
    def generate_report(self):
        ...
        # Save raw data
        json_file = os.path.join(self.output_dir, f'performance_metrics_{timestamp}.json')
    ...
    def generate_summary(self, timestamp):
        ...
        report_file = os.path.join(self.output_dir, f'performance_summary_{timestamp}.txt')
        ...
        with open(report_file, 'w') as f:
            ...
            for key in ['lidar1_rate', 'lidar2_rate', 'detection1_rate', 'detection2_rate']:
                if self.metrics[key]:
                    rates = list(self.metrics[key])
                    f.write(f"  {key}:\n")
            ...
            if self.metrics['processing_latencies']:
                ...
                f.write(f"  Average: {np.mean(self.metrics['processing_latencies']):.1f} ms\n")
                f.write(f"  Median: {np.median(self.metrics['processing_latencies']):.1f} ms\n")
                f.write(f"  95th percentile: {np.percentile(self.metrics['processing_latencies'], 95):.1f} ms\n")
                f.write(f"  Max: {np.max(self.metrics['processing_latencies']):.1f} ms\n\n")
            
            # Calibrations
            f.write(f"Calibrations:\n")
            f.write(f"  Total: {self.metrics['calibration_count']}\n")
            if self.duration_sec > 0:
                f.write(f"  Rate: {self.metrics['calibration_count'] / self.duration_sec * 60:.2f} per minute\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='Profile multi_wayside_node performance')
