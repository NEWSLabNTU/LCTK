...
class SyntheticDataGenerator(Node):
    ...
    def create_board_points(self, center: Tuple[float, float, float], 
                          size: float = 1.0, 
        ...
        for _ in range(num_points):
            ...
            hole_positions = [(-0.3*size, -0.3*size), (0.3*size, -0.3*size), (0.0, 0.3*size)]
    ...
    def create_background_points(self, num_points: int = 5000, 
                               bounds: Tuple[float, float, float, float, float, float] = (-10, 10, -10, 10, -2, 5)) -> np.ndarray:
    ...
    def points_to_pointcloud2(self, points: np.ndarray, frame_id: str, timestamp) -> PointCloud2:
        ...
        data = []
        for point in points:
            data.append(struct.pack('ffff', float(point[0]), float(point[1]), float(point[2]), float(point[3])))
    ...
    def generate_scenario_perfect_boards(self, output_dir: str):
        ...
        lidar1_topic = rosbag2_py.TopicMetadata(
            name='/lidar1/points', type='sensor_msgs/msg/PointCloud2', serialization_format='cdr')
        lidar2_topic = rosbag2_py.TopicMetadata(
            name='/lidar2/points', type='sensor_msgs/msg/PointCloud2', serialization_format='cdr')
        ...
        for i in range(50):
            ...
            board_points_1 = self.create_board_points(board_position, size=1.0, num_points=2000)
            background_1 = self.create_background_points(1000)
            all_points_1 = np.vstack([board_points_1, background_1])
            
            pc_msg_1 = self.points_to_pointcloud2(all_points_1, 'test_lidar1', timestamp)
            
            # LiDAR 2 point cloud (transform board to LiDAR 2 frame)
            board_points_2 = self.create_board_points(board_position, size=1.0, num_points=2000)
            ...
            pc_msg_2 = self.points_to_pointcloud2(all_points_2, 'test_lidar2', timestamp)
            
            # Write to bag
            writer.write('/lidar1/points', serialize_message(pc_msg_1), int(timestamp.sec * 1e9 + timestamp.nanosec))
            writer.write('/lidar2/points', serialize_message(pc_msg_2), int(timestamp.sec * 1e9 + timestamp.nanosec))
    ...
    def generate_scenario_noisy_data(self, output_dir: str):
        ...
        for i in range(50):
            ...
            # LiDAR 1 with noise
            board_points_1 = self.create_board_points(noisy_position, size=1.0, num_points=1500)
            # Add measurement noise
            board_points_1[:, :3] += np.random.normal(0, 0.005, (len(board_points_1), 3))
            # Add some outliers
            ...
            board_points_2 = self.create_board_points(noisy_position, size=1.0, num_points=1200)
            # Different noise level
            board_points_2[:, :3] += np.random.normal(0, 0.008, (len(board_points_2), 3))
    ...
    def generate_scenario_partial_occlusion(self, output_dir: str):
        ...
        for i in range(50):
            ...
            for idx in range(len(board_points_1)):
                x, y = board_points_1[idx, 0] - board_position[0], board_points_1[idx, 1] - board_position[1]
                if (x - occlusion_center[0])**2 + (y - occlusion_center[1])**2 < occlusion_radius**2:
                    mask[idx] = False
            ...
            for idx in range(len(board_points_2)):
                x, y = board_points_2[idx, 0] - board_position[0], board_points_2[idx, 1] - board_position[1]
                if (x - occlusion_center_2[0])**2 + (y - occlusion_center_2[1])**2 < occlusion_radius_2**2:
                    mask2[idx] = False
    ...
    def generate_scenario_multi_board(self, output_dir: str):
        ...
        for i in range(50):
            ...
            for idx, pos in enumerate(board_positions):
                ...
                board_points = self.create_board_points(pos, size=1.0, num_points=num_points)
                
                # Add varying noise levels
                noise_level = 0.003 if idx == 0 else 0.01
                board_points[:, :3] += np.random.normal(0, noise_level, (len(board_points), 3))
...
def main():
    parser = argparse.ArgumentParser(description='Generate synthetic test data for multi_wayside_node')
    parser.add_argument('--output_dir', type=str, default='test_data',
                        help='Output directory for generated bags')
    parser.add_argument('--scenarios', nargs='+', default=['perfect'],
