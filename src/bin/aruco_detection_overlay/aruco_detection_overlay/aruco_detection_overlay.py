...
class DetectionOverlayNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('detection_topic', '/calibration/aruco_locator/aruco_detections')
        self.declare_parameter('output_topic', '/calibration/visualization/image_with_detections')
    ...
    def detection_callback(self, msg):
        ...
        with self.cache_lock:
            ...
            if self.detection_count % 10 == 0:
                self.get_logger().info(f'Received {self.detection_count} detection messages')
    ...
    def find_matching_detection(self, image_stamp):
        ...
        with self.cache_lock:
            ...
            for detection in self.detection_cache:
                detection_time = detection.header.stamp.sec * 1e9 + detection.header.stamp.nanosec
        ...
        try:
            ...
            if detection_msg and detection_msg.detections:
                ...
                for i, detection in enumerate(detection_msg.detections):
                    ...
                    # Draw label text
                    cv2.putText(cv_image, label, (x1 + 5, label_y),
                ...
                cv2.putText(cv_image, detection_text, (10, 40),
                           cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)
            else:
                # No detections overlay
                cv2.putText(cv_image, "No ArUco markers detected", (10, 40),
