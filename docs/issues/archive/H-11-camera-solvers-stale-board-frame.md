...
- **Area:** `ros/extrinsic_solver_node`, `ros/advanced_extrinsic_solver` (renamed
  ...
  convention
- **Status:** Fixed (2026-09-01) — the defect was real when filed; Stage 1 fixed every reachable camera-calibration path and Stage 3 removed the superseded implementation that still carried it
...
- **Related:** [M-20](./M-20-board-frame-edge-aligned-vs-diamond-naming.md) (the Phase 1 issue), [M-14](./M-14-corner-order-brittle.md) (the duplicated corner layout — closed by the Phase 2 geometry extraction), [M-12](./M-12-no-robust-estimation-or-refinement.md) (the estimator asymmetry — closed by Phase 2 Stage 2), [C-01](./C-01-aruco-corners-discarded.md), [H-10](./H-10-dump-load-regresses-c01.md) (the saved-file format this bumps to version 4), [L-22](./L-22-advanced-solver-undeclared-lctk-interfaces-dep.md) and [L-23](../L-23-debug-mode-parameter-never-read.md) (found while scoping the fix)
...
1. **A 45° in-plane rotation — silent.** The marker grid is a symmetric 2×2, so a quarter-turn's
   ...
   [H-09](./H-09-no-extrinsic-quality-metric.md)'s central point: the system has no way to
...
4. **Bump the saved-detection format to version 4**, carrying that same identifier, so a
   ...
   with a loud warning, in the manner [H-10](./H-10-dump-load-regresses-c01.md) established.
...
([M-20](./M-20-board-frame-edge-aligned-vs-diamond-naming.md), fixed): the published board
...
## Resolution (2026-09-01) — Stage 3 complete

The defect described here was real when filed. Phase 1 changed the LiDAR board pose to the
corner-aligned, plate-centre frame while the Python camera solvers still supplied marker points in
the old edge-aligned, corner-origin frame. That mismatch could produce a silent 45° in-plane error
and an approximately 707 mm origin shift.

Stage 1 of the linked specification ported the maintained camera path to the new convention. The
renamed `lidar_to_camera_solver` now consumes the shared `lctk_target` geometry, whose marker-corner
contract is covered by the cross-language world-coordinate golden. Stage 2 moved both continuous
and manual operation onto that corrected backend. Therefore the original frame defect was removed
from every supported, config-driven calibration path before this resolution.

Stage 3 now deletes `ros/extrinsic_solver_node/`, rather than porting it. That node was already
unreachable from the config-driven launch and justfile workflows; it also depended on the deleted
legacy ArUco board schema and its own launch file did not provide the required
`aruco_config_file`. Deleting it removes the last stale implementation and its workspace, package,
documentation, and overlay references.

The old title and diagnosis describe the pre-Stage-1 state. They remain useful as the incident
record, but must not be read as saying that the maintained `lidar_to_camera_solver` still uses the
old frame.
